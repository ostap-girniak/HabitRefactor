    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center space-y-6 animate-fade-in">
        ...
        <div className="max-w-md space-y-2">
          <h1 className="text-3xl font-black text-[var(--text-primary)]">The Map is Empty</h1>
          <p className="text-[var(--text-secondary)]">
            You haven't declared war on any habits yet. A warrior's journey begins with the first battle.
          </p>
        </div>
        <Link href="/habits/new" className="btn-fire px-8 py-4 text-lg">
          Declare Your First Battle
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div>
        ...
        <p className="text-[var(--text-secondary)] text-sm mt-1">
          {activeHabit ? `Focusing on: ${activeHabit.name}` : "Global behavioral overview."}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card text-center py-6 border-[rgba(0,230,118,0.15)] bg-gradient-to-br from-[rgba(0,230,118,0.03)] to-transparent">
          ...
          <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[0.2em] font-bold">Savings</div>
        </div>
        ...
        <div className="card text-center py-6 border-[rgba(68,138,255,0.15)] bg-gradient-to-br from-[rgba(68,138,255,0.03)] to-transparent">
          ...
          <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[0.2em] font-bold">Time</div>
        </div>
        ...
        <div className="card text-center py-6 border-[rgba(255,140,0,0.15)] bg-gradient-to-br from-[rgba(255,140,0,0.03)] to-transparent">
          ...
          <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-[0.2em] font-bold">Battles</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card space-y-4">
            ...
            <p className="text-xs text-[var(--text-secondary)] text-center">
              Identifies which days you are most statistically vulnerable.
            </p>
          </div>
              ...
              <div className="pt-3 border-t border-[var(--border-default)]">
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <div className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded border ${
                    ...
                    Risk {Math.round(oracleData.forecast.risk_score)}%
                  </div>
                </div>
        ...
        {oracleData?.top_triggers?.length > 0 && (
          <div className="card">
            <div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-3">
              Predatory Triggers (Most Frequent)
            </div>
        ...
        <div className="card space-y-4">
          ...
          <div className="flex justify-center gap-4 text-[10px] font-bold uppercase tracking-wider">
            <span className="flex items-center gap-1 text-[var(--accent-info)]">
              <div className="w-2 h-2 rounded-full bg-[var(--accent-info)]" /> Mood
            </span>
            <span className="flex items-center gap-1 text-[var(--accent-danger)]">
              <div className="w-2 h-0.5 bg-[var(--accent-danger)]" /> Stress
            </span>
          </div>
        </div>
      </div>
              ...
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-default)]">
                <div>
                  <div className="font-bold text-sm text-[var(--text-primary)]">
                    {battle.habit_name}: {battle.duration_days} Day Clean Streak
                  </div>
                  <div className="text-[10px] text-[var(--text-muted)] mt-1 flex items-center gap-2">
                    <span>Ended: {new Date(battle.end_date).toLocaleDateString()}</span>
                    {battle.trigger && (
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[var(--text-primary)]">Your Battles</h2>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-[var(--text-muted)] mt-0.5 truncate">
                  Best: {habit.best_streak_days || 0}d • {formatMoney((habit.current_streak_days || 0) * (habit.cost_per_unit || 0))} saved
                </div>
              </div>
      ...
      {latestInsight ? (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="badge badge-fire">New Insight</span>
        ...
        <div className="card bg-[var(--bg-secondary)] border-dashed border-2 flex items-center justify-center py-10 opacity-60">
          <p className="text-[var(--text-muted)] text-sm">
            AI is analyzing your patterns. Stay consistent to unlock insights.
          </p>
        </div>
      )}
    </div>
